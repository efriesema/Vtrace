...
def make_mb_discounts(shape,gamma):
    #Make a TxB tensor with gamma discount levels per timestep across the minibatch
    #shape is a tuple defining shape of discounts matrix
    discounts = np.ones(shape = shape)
    
    (rows,col) = shape
    for i in range(rows):
        discounts[i:,:]=gamma*discounts[i:,:]  
    return discounts
